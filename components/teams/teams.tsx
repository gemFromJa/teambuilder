...
export default function Teams({
  ...
  const availableColors = useMemo(
    () =>
      colors.filter(
        (color) => teams.findIndex((team) => team.color === color) === -1
      ),
    [teams]
  );
  ...
  return (
    <div>
      ...
      <div className="mb-4 grid box-border justify-between grid-cols-[repeat(auto-fit,minmax(16rem,1fr))] gap-x-16 gap-y-3">
        {teams.map((team, idx) => (
          <div
            key={`team_${idx}`}
            className={`self-stretch mb-2 w-full flex gap-3 border-2 py-1 px-2 border-gray-100 rounded-md focus-within:outline-double`}
          >
            {/* <div className={`flex justify-between min-w-[50%] w-[100%] mb-1`}>
              <label>Team Info</label>
              <div
                aria-disabled={teams.length <= 2}
                onClick={() => teams.length > 2 && removeTeam(idx)}
                className="cursor-pointer ml-4 h-full"
              >
                <img className="h-[16px]" src="/images/delete.png" />
              </div>
            </div> */}
            {/* <div
              className={`flex gap-3 border-2 px-2 border-gray-100 rounded-md focus-within:outline-double`}
            > */}
            ...
            <div className="flex-grow">
              <input
                className={`px-3 py-2 h-full w-full flex-1 outline-none`}
                width={30}
                value={team.name}
                onChange={(e) => {
                  updateTeams({
      ...
      <div
        onClick={addTeam}
        className="inline-flex gap-3 items-center justify-center px-3 cursor-pointer group"
      >
        <img src="/images/plus.png" className="h-[16px] inline-block" />
        <span className="text-gray-400 text-sm group-hover:text-gray-800">
          New Team
        </span>
      </div>
    </div>
...
const ColorSelectorDropdown = ({
  ...
  return (
    ...
    >
      <div className="h-full flex gap-3 items-center max-w-[87px] px-3 py-2 cursor-pointer">
      ...
      >
        {availableColors.map((color, key) => (
          <div
            ...
            className={`grid grid-cols-[18px_auto] items-center gap-2 py-2`}
          >
...
export const ColorPreview = ({ color }: { color: Color }) => (
  <div
    ...
    className={`w-[18px] h-[18px] rounded-full border self-center flex-grow-0`}
  />
);
