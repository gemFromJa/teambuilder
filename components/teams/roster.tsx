  ...
  return (
    <div className="h-full flex-col flex gap-4">
      <div className="flex justify-between mbd-4 text-sm">
        ...
        />
        <button onClick={() => {}} disabled className="disabled:text-gray-200">
          Done
        </button>
      </div>
      <div
          ...
          {teams.map((team, i) => {
            return (
              <div key={`team_${i}`} className="text-base">
                ...
                <span>
                  {team.players.reduce(
                    (sum, player) => `${sum}${sum ? ", " : ""}${player?.name}`,
                    ""
                  )}
                </span>
              </div>
      ...
      {teamFormation.map((playerRow, i) => {
        return (
          <PlayersRow
            ...
            height={`${rowHeight}%`}
            players={playerRow}
            onSelectPlayer={(index: number) => onSelectPlayer(i, index)}
            teamIndex={teamIndex}
          />
        );
      })}
